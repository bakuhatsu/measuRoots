...
#' @export
#'
mergeCSVs <- function(csvFile = file.choose(), returnFile = F) { # select a file in the target folder
  ...
  if (grepl("WW[1-3]", csvFile)) { # convert WW# to WW
    csvFile <- sub("WW[1-3]", "WW", csvFile)
  } else if (grepl("WS[1-3]", csvFile)) { # WS1-3 --> WS
    csvFile <- sub("WS[1-3]", "WS", csvFile)
  }
  if (grepl(" s[1-8]", csvFile)) { # get rid of s1 - s8
    csvFile <- sub(" s[1-8]", "", csvFile)
  }
  if (grepl(".csv", csvFile)) {
    csvFile <- sub(".csv", " (combined).csv", csvFile)
  }
  # output a file
